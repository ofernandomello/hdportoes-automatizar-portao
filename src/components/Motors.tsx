...
const Motors = () => {
  ...
  const motors = [
    ...
    },
    {
      ...
      features: [
        "Portões até 300kg",
        "3 controles remotos inclusos",
        "Garantia de 24 meses",
        "Instalação profissional",
        "Sensor de segurança duplo",
        "Abertura em 4 segundos",
    ...
    },
    {
      ...
      features: [
        "Portões até 500kg",
        "4 controles remotos inclusos",
        "Garantia de 36 meses",
        "Instalação profissional",
        "Sistema completo de segurança",
        "Abertura em 3 segundos",
        "Motor silencioso",
        "Automação via smartphone"
      ],
      isHighlight: false
    }
  ];
  ...
  return (
    <section id="motores" className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Trabalhamos com as melhores marcas do mercado. Parcelamento em até 10x sem juros.
          </p>
        </div>
          ...
          {motors.map((motor, index) => (
            <div 
              ...
              className={`relative rounded-2xl p-8 transition-all ${
                motor.isHighlight 
                  ? 'bg-primary text-primary-foreground shadow-[0_20px_50px_-10px_hsl(142_76%_36%/0.4)] scale-105 lg:-my-4' 
                  : 'bg-card text-card-foreground shadow-[0_4px_20px_-2px_hsl(0_0%_0%/0.08)] hover:shadow-[0_8px_30px_-4px_hsl(0_0%_0%/0.12)]'
              }`}
            >
              {motor.isHighlight && (
                <Badge className="absolute -top-3 left-1/2 -translate-x-1/2 bg-foreground text-background px-4 py-1 font-montserrat font-semibold">
              ...
              <ul className="space-y-4 mb-8">
                {motor.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="flex items-start gap-3">
                    <Check className={`h-5 w-5 flex-shrink-0 mt-0.5 ${
                      motor.isHighlight ? 'text-primary-foreground' : 'text-primary'
                    }`} />
              ...
              <Button 
                className={`w-full font-montserrat font-semibold py-6 text-lg rounded-xl transition-all ${
                  motor.isHighlight
                    ? 'bg-background text-foreground hover:bg-background/90 shadow-lg'
                    : 'bg-primary text-primary-foreground hover:bg-primary/90'
                }`}
                onClick={() => handleWhatsApp(motor.name)}
              >
                Solicitar Orçamento
              </Button>
            </div>
